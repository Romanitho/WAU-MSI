Function Start-WixBuild {
  ...
  Param(
    ...
    [Parameter(Mandatory = $false)]  [string] $DialogFile = "$($Path)\files\dialog.bmp",
    [Parameter(Mandatory = $false)]  [string] $productId = "WAU",
    [Parameter(Mandatory = $false)]  [string] $ProductName = "Winget-AutoUpdate",
    [Parameter(Mandatory = $false)]  [string] $ProductVersion = "2.0.0",
...
"@
    ...
    & $heatExe dir $path -nologo -sfrag -sw5151 -ag -srd -gg -dir $ProductName -out $productWxs -cg INSTALLDIR -dr INSTALLDIR | Out-Null
  ...
  foreach ($platform in $platforms) {
    ...
    & $lightexe -sval -sw1076 -spdb -ext WixUIExtension -out $platformOutputMsi $modulesWixobj $productWixobj -b $path -sice:ICE91 -sice:ICE69 -sice:ICE38 -sice:ICE57 -sice:ICE64 -sice:ICE204 -sice:ICE80 | Out-Null
  }
}
