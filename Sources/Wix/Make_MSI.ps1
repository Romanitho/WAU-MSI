...
Param(
  ...
  [Parameter(Mandatory = $false)]  [string] $IconFile = "$($Path)\Wix\files\icon.ico",
  [Parameter(Mandatory = $false)]  [string] $BannerFile = "$($Path)\Wix\files\banner.bmp",
  [Parameter(Mandatory = $false)]  [string] $DialogFile = "$($Path)\Wix\files\dialog.bmp",
