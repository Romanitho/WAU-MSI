    ...
    if ($SessionID -eq 0) {
        ...
        if ($ServiceUI) {
            ...
            if ($explorerprocesses.Count -gt 0) {
                ...
                Start-Process "ServiceUI.exe" -ArgumentList "-process:explorer.exe $env:windir\System32\wscript.exe Invisible.vbs ""\""powershell.exe -NoProfile -ExecutionPolicy Bypass -File winget-upgrade.ps1""\""" -WorkingDirectory $WorkingDir -Wait
                Exit 0
            }
            else {
